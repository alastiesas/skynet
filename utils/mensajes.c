...
void* recibir_mensaje(uint32_t socket_cliente, uint32_t* size, t_log* logger)
{
	...
	if(recv_with_retry(socket_cliente, size, sizeof(uint32_t), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, buffer, *size, MSG_WAITALL) == -1)
...
int32_t send_ID(int32_t ID, uint32_t socket, t_log* logger){
	...
	if((result = send_with_retry(socket, &ID, sizeof(int32_t), MSG_NOSIGNAL)) == -1)
...
uint32_t receive_size(uint32_t socket, t_log* logger){
	...
	if(recv_with_retry(socket, &size, sizeof(uint32_t), MSG_WAITALL) == -1)
...
uint32_t receive_ID_proceso(uint32_t socket, t_log* logger){
	...
	int32_t resultado;
	if((resultado = recv_with_retry(socket, &ID_proceso, sizeof(uint32_t), MSG_WAITALL)) == -1){
...
t_message_new* receive_new(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	//recibir id de new
	if(recv_with_retry(socket_cliente, &(new->id), sizeof(new->id), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el id de new");
	else
		log_info(logger, "id de new recibido: %d", new->id);

	//recibir size_nombre de new
	if(recv_with_retry(socket_cliente, &(new->size_pokemon_name), sizeof(new->size_pokemon_name), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, new->pokemon_name, new->size_pokemon_name, MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el nombre de new");
	else{
		memcpy(new->pokemon_name + new->size_pokemon_name, &fin, 1);
		log_info(logger, "nombre de new recibido: %s", new->pokemon_name);
	}

	//recibir posX de new
	if(recv_with_retry(socket_cliente, &(new->location->position->x), sizeof(new->location->position->x), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir la posX de new");
	else
		log_info(logger, "posX de new recibida: %d", new->location->position->x);

	//recibir posY de new
	if(recv_with_retry(socket_cliente, &(new->location->position->y), sizeof(new->location->position->y), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(new->location->amount), sizeof(new->location->amount), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir la cantidad de new");
	else
		log_info(logger, "cantidad de new recibida: %d\n", new->location->amount);


	if(*size != sizeof(new->id) + sizeof(new->size_pokemon_name) + strlen(new->pokemon_name) + sizeof(new->location->position->x) + sizeof(new->location->position->y) + sizeof(new->location->amount))
...
t_message_appeared* receive_appeared(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	if(recv_with_retry(socket_cliente, &(appeared->id), sizeof(appeared->id), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(appeared->correlative_id), sizeof(appeared->correlative_id), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el id de appeared");
	else
		log_info(logger, "correlative_id de appeared recibido: %d", appeared->correlative_id);

	//recibir size_pokemon_name de appeared
	if(recv_with_retry(socket_cliente, &(appeared->size_pokemon_name), sizeof(appeared->size_pokemon_name), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, appeared->pokemon_name, appeared->size_pokemon_name, MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el nombre de appeared");
	else{
		memcpy(appeared->pokemon_name + appeared->size_pokemon_name, &fin, 1);
		log_info(logger, "nombre de appeared recibido: %s", appeared->pokemon_name);
	}

	//recibir posX de appeared
	if(recv_with_retry(socket_cliente, &(appeared->position->x), sizeof(appeared->position->x), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(appeared->position->y), sizeof(appeared->position->y), MSG_WAITALL) == -1)
	...
	if(*size != sizeof(appeared->id) + sizeof(appeared->correlative_id) + sizeof(appeared->size_pokemon_name) + strlen(appeared->pokemon_name) + sizeof(appeared->position->x) + sizeof(appeared->position->y))
		log_error(logger, "Tamanio erroneo");
...
t_message_get* receive_get(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	//recibir id de get
	if(recv_with_retry(socket_cliente, &(get->id), sizeof(get->id), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(get->size_pokemon_name), sizeof(get->size_pokemon_name), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, get->pokemon_name, get->size_pokemon_name, MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el nombre de get");
	else{
		memcpy(get->pokemon_name + get->size_pokemon_name, &fin, 1);
		log_info(logger, "nombre de appeared recibido: %s", get->pokemon_name);
	}

	if(*size != sizeof(get->id) + sizeof(get->size_pokemon_name) + strlen(get->pokemon_name))
		log_error(logger, "Tamanio erroneo");
...
t_message_localized* receive_localized(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	if(recv_with_retry(socket_cliente, &(localized->id), sizeof(localized->id), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(localized->correlative_id), sizeof(localized->correlative_id), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el id de localized");
	else
		log_info(logger, "correlative_id de localized recibido: %d", localized->correlative_id);

	//recibir size_pokemon_name de localized
	if(recv_with_retry(socket_cliente, &(localized->size_pokemon_name), sizeof(localized->size_pokemon_name), MSG_WAITALL) == -1)
	...
	//recibir nombre de appeared
	if(recv_with_retry(socket_cliente, localized->pokemon_name, localized->size_pokemon_name, MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el nombre de localized");
	else{
		memcpy(localized->pokemon_name + localized->size_pokemon_name, &fin, 1);
	...
	for(int i = 0; i < localized->position_amount; i++) {
		...
		if(recv_with_retry(socket_cliente, &(localized->positions[i].x), sizeof(localized->positions[i].x), MSG_WAITALL) == -1)
		...
		if(recv_with_retry(socket_cliente, &(localized->positions[i].y), sizeof(localized->positions[i].y), MSG_WAITALL) == -1)
	...
	if(*size != sizeof(localized->id) + sizeof(localized->correlative_id) + sizeof(localized->size_pokemon_name) + strlen(localized->pokemon_name) + sizeof(localized->position_amount) + (sizeof(localized->positions[0].x) + sizeof(localized->positions[0].y))*localized->position_amount)
		log_error(logger, "Tamanio erroneo");
...
t_message_catch* receive_catch(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	//recibir id de catch
	if(recv_with_retry(socket_cliente, &(catch->id), sizeof(catch->id), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(catch->size_pokemon_name), sizeof(catch->size_pokemon_name), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, catch->pokemon_name, catch->size_pokemon_name, MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el nombre de catch");
	else{
		memcpy(catch->pokemon_name + catch->size_pokemon_name, &fin, 1);
		log_info(logger, "nombre de catch recibido: %s", catch->pokemon_name);
	}

	//recibir posX de catch
	if(recv_with_retry(socket_cliente, &(catch->position->x), sizeof(catch->position->x), MSG_WAITALL) == -1)
	...
	if(recv_with_retry(socket_cliente, &(catch->position->y), sizeof(catch->position->y), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir ela posY de catch");
	else
		log_info(logger, "posY de catch recibida: %d\n", catch->position->y);


	if(*size != sizeof(catch->id) + sizeof(catch->size_pokemon_name) + strlen(catch->pokemon_name) + sizeof(catch->position->x) + sizeof(catch->position->y))
		log_error(logger, "Tamanio erroneo");
...
t_message_caught* receive_caught(uint32_t socket_cliente, uint32_t* size, t_log* logger){
	...
	//recibir id de appeared
	if(recv_with_retry(socket_cliente, &(caught->id), sizeof(caught->id), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el id de appeared");
	else
		log_info(logger, "id de caught recibido: %d", caught->id);

	//recibir correlative_id de appeared
	if(recv_with_retry(socket_cliente, &(caught->correlative_id), sizeof(caught->correlative_id), MSG_WAITALL) == -1)
		log_error(logger, "Error al recibir el id de appeared");
	else
		log_info(logger, "correlative_id de caught recibido: %d", caught->correlative_id);

	//recibir resultado de appeared
		if(recv_with_retry(socket_cliente, &(caught->result), sizeof(caught->result), MSG_WAITALL) == -1)
			log_error(logger, "Error al recibir el id de appeared");
		else
